...
trait TypeDeclEMT extends AnyRef with AstNodeEMT with HasAliasTypeFullNameEMT with HasAstParentFullNameEMT with HasAstParentTypeEMT with HasFilenameEMT with HasFullNameEMT with HasGenericSignatureEMT with HasInheritsFromTypeFullNameEMT with HasIsExternalEMT with HasNameEMT
...
*▸ AliasTypeFullName (String); Cardinality `ZeroOrOne` (optional); This property holds the fully qualified name of the type that the node is
a type alias of.
...
*▸ InheritsFromTypeFullName (String); Cardinality `List` (many); The static types a TYPE_DECL inherits from. This property is matched against the
...
order of 0.*/
class TypeDecl(graph_4762: flatgraph.Graph, seq_4762: Int) extends StoredNode(graph_4762, 40, seq_4762) with TypeDeclBase with AstNode with StaticType[TypeDeclEMT] {
