...
val tmpEvidenceDescription = this.evidenceDescription; if(tmpEvidenceDescription.nonEmpty) res.put("EVIDENCE_DESCRIPTION", tmpEvidenceDescription)
...
*▸ EvidenceDescription (String); Cardinality `List` (many); Optional description for nodes in evidence. Used to give a hint about the kind of evidence
...
class Finding(graph_4762: flatgraph.Graph, seq_4762: Int) extends StoredNode(graph_4762, 15, seq_4762) with FindingBase with StaticType[FindingEMT] {
  def evidence: IndexedSeq[StoredNode] = flatgraph.Accessors.getNodePropertyMulti[StoredNode](graph, nodeKind = nodeKind, propertyKind = 51, seq = seq)
def keyValuePairs: IndexedSeq[KeyValuePair] = flatgraph.Accessors.getNodePropertyMulti[KeyValuePair](graph, nodeKind = nodeKind, propertyKind = 52, seq = seq)
