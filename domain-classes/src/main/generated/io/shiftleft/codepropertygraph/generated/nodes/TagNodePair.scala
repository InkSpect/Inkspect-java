...
class TagNodePair(graph_4762: flatgraph.Graph, seq_4762: Int) extends StoredNode(graph_4762, 36, seq_4762) with TagNodePairBase with StaticType[TagNodePairEMT] {
  def node: StoredNode = flatgraph.Accessors.getNodePropertySingle(graph, nodeKind = nodeKind, propertyKind = 53, seq = seq, default = null: StoredNode)
def tag: Tag = flatgraph.Accessors.getNodePropertySingle(graph, nodeKind = nodeKind, propertyKind = 54, seq = seq, default = null: Tag)
