...
trait MemberEMT extends AnyRef with AstNodeEMT with DeclarationEMT with HasAstParentFullNameEMT with HasAstParentTypeEMT with HasDynamicTypeHintFullNameEMT with HasGenericSignatureEMT with HasPossibleTypesEMT with HasTypeFullNameEMT
...
*▸ DynamicTypeHintFullName (String); Cardinality `List` (many); Type hint for the dynamic type. These are observed to be verifiable at runtime.
...
*▸ PossibleTypes (String); Cardinality `List` (many); Similar to `DYNAMIC_TYPE_HINT_FULL_NAME`, but that this makes no guarantee that types within this property are correct. This property is used to capture observations between node interactions during a 'may-analysis'.

*▸ TypeFullName (String); Cardinality `one` (mandatory with default value `<empty>`); This field contains the fully-qualified static type name of the program
...
class Member(graph_4762: flatgraph.Graph, seq_4762: Int) extends StoredNode(graph_4762, 23, seq_4762) with MemberBase with AstNode with Declaration with StaticType[MemberEMT] {
