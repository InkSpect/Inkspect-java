...
trait MethodEMT extends AnyRef with CfgNodeEMT with DeclarationEMT with HasAstParentFullNameEMT with HasAstParentTypeEMT with HasColumnNumberEndEMT with HasFilenameEMT with HasFullNameEMT with HasGenericSignatureEMT with HasHashEMT with HasIsExternalEMT with HasLineNumberEndEMT with HasSignatureEMT
...
*▸ AstParentFullName (String); Cardinality `one` (mandatory with default value `<empty>`); This field holds the FULL_NAME of the AST parent of an entity.

*▸ AstParentType (String); Cardinality `one` (mandatory with default value `<empty>`); The type of the AST parent. Since this is only used in some parts of the graph,
the list does not include all possible parents by intention.
Possible parents: METHOD, TYPE_DECL, NAMESPACE_BLOCK.

*▸ Code (String); Cardinality `one` (mandatory with default value `<empty>`); This field holds the code snippet that the node represents.

*▸ ColumnNumber (Int); Cardinality `ZeroOrOne` (optional); This optional fields provides the column number of the program construct
represented by the node.

*▸ ColumnNumberEnd (Int); Cardinality `ZeroOrOne` (optional); This optional fields provides the column number at which the program construct
represented by the node ends.

*▸ Filename (String); Cardinality `one` (mandatory with default value `<empty>`); The path of the source file this node was generated from, relative to the root
...
*▸ FullName (String); Cardinality `one` (mandatory with default value `<empty>`); This is the fully-qualified name of an entity, e.g., the fully-qualified
...
*▸ GenericSignature (String); Cardinality `one` (mandatory with default value `<empty>`); This field is experimental. It will likely be removed in the future without any notice.
...
*▸ IsExternal (Boolean); Cardinality `one` (mandatory with default value `false`); Indicates that the construct (METHOD or TYPE_DECL) is external, that is,
...
*▸ LineNumber (Int); Cardinality `ZeroOrOne` (optional); This optional field provides the line number of the program construct
represented by the node.

*▸ LineNumberEnd (Int); Cardinality `ZeroOrOne` (optional); This optional fields provides the line number at which the program construct
represented by the node ends.

*▸ Name (String); Cardinality `one` (mandatory with default value `<empty>`); Name of represented object, e.g., method name (e.g. "run")

*▸ Offset (Int); Cardinality `ZeroOrOne` (optional); Start offset into the CONTENT property of the corresponding FILE node.
...
*▸ OffsetEnd (Int); Cardinality `ZeroOrOne` (optional); End offset (exclusive) into the CONTENT property of the corresponding FILE node.
...
*▸ Order (Int); Cardinality `one` (mandatory with default value `-1`); This integer indicates the position of the node among
its siblings in the AST. The left-most child has an
order of 0.

*▸ Signature (String); Cardinality `one` (mandatory with default value ``); The method signature encodes the types of parameters in a string.
...
class Method(graph_4762: flatgraph.Graph, seq_4762: Int) extends StoredNode(graph_4762, 25, seq_4762) with MethodBase with CfgNode with Declaration with StaticType[MethodEMT] {
