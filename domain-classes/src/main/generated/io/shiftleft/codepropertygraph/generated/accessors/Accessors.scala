...
object Accessors {
  ...
  def aliasTypeFullName: Option[String]  = flatgraph.Accessors.getNodePropertyOption[String](node.graph, nodeKind = node.nodeKind, propertyKind = 0, seq = node.seq)
}
final class AccessPropertyArgumentIndex(val node: nodes.StoredNode) extends AnyVal {
  def argumentIndex: Int  = flatgraph.Accessors.getNodePropertySingle(node.graph, nodeKind = node.nodeKind, propertyKind = 1, seq = node.seq(), default = -1: Int)
}
final class AccessPropertyArgumentName(val node: nodes.StoredNode) extends AnyVal {
  def argumentName: Option[String]  = flatgraph.Accessors.getNodePropertyOption[String](node.graph, nodeKind = node.nodeKind, propertyKind = 2, seq = node.seq)
}
final class AccessPropertyAstParentFullName(val node: nodes.StoredNode) extends AnyVal {
  def astParentFullName: String  = flatgraph.Accessors.getNodePropertySingle(node.graph, nodeKind = node.nodeKind, propertyKind = 3, seq = node.seq(), default = "<empty>": String)
}
final class AccessPropertyAstParentType(val node: nodes.StoredNode) extends AnyVal {
  def astParentType: String  = flatgraph.Accessors.getNodePropertySingle(node.graph, nodeKind = node.nodeKind, propertyKind = 4, seq = node.seq(), default = "<empty>": String)
}
final class AccessPropertyCanonicalName(val node: nodes.StoredNode) extends AnyVal {
  def canonicalName: String  = flatgraph.Accessors.getNodePropertySingle(node.graph, nodeKind = node.nodeKind, propertyKind = 5, seq = node.seq(), default = "<empty>": String)
}
final class AccessPropertyClosureBindingId(val node: nodes.StoredNode) extends AnyVal {
  def closureBindingId: Option[String]  = flatgraph.Accessors.getNodePropertyOption[String](node.graph, nodeKind = node.nodeKind, propertyKind = 6, seq = node.seq)
}
final class AccessPropertyCode(val node: nodes.StoredNode) extends AnyVal {
  def code: String  = flatgraph.Accessors.getNodePropertySingle(node.graph, nodeKind = node.nodeKind, propertyKind = 7, seq = node.seq(), default = "<empty>": String)
}
final class AccessPropertyColumnNumber(val node: nodes.StoredNode) extends AnyVal {
  def columnNumber: Option[Int]  = flatgraph.Accessors.getNodePropertyOption[Int](node.graph, nodeKind = node.nodeKind, propertyKind = 8, seq = node.seq)
}
final class AccessPropertyColumnNumberEnd(val node: nodes.StoredNode) extends AnyVal {
  def columnNumberEnd: Option[Int]  = flatgraph.Accessors.getNodePropertyOption[Int](node.graph, nodeKind = node.nodeKind, propertyKind = 9, seq = node.seq)
}
final class AccessPropertyContainedRef(val node: nodes.StoredNode) extends AnyVal {
  def containedRef: String  = flatgraph.Accessors.getNodePropertySingle(node.graph, nodeKind = node.nodeKind, propertyKind = 10, seq = node.seq(), default = "<empty>": String)
}
final class AccessPropertyContent(val node: nodes.StoredNode) extends AnyVal {
  def content: String  = flatgraph.Accessors.getNodePropertySingle(node.graph, nodeKind = node.nodeKind, propertyKind = 11, seq = node.seq(), default = "<empty>": String)
}
final class AccessPropertyControlStructureType(val node: nodes.StoredNode) extends AnyVal {
  def controlStructureType: String  = flatgraph.Accessors.getNodePropertySingle(node.graph, nodeKind = node.nodeKind, propertyKind = 12, seq = node.seq(), default = "<empty>": String)
}
final class AccessPropertyDependencyGroupId(val node: nodes.StoredNode) extends AnyVal {
  def dependencyGroupId: Option[String]  = flatgraph.Accessors.getNodePropertyOption[String](node.graph, nodeKind = node.nodeKind, propertyKind = 13, seq = node.seq)
}
final class AccessPropertyDispatchType(val node: nodes.StoredNode) extends AnyVal {
  def dispatchType: String  = flatgraph.Accessors.getNodePropertySingle(node.graph, nodeKind = node.nodeKind, propertyKind = 14, seq = node.seq(), default = "<empty>": String)
}
final class AccessPropertyDynamicTypeHintFullName(val node: nodes.StoredNode) extends AnyVal {
  def dynamicTypeHintFullName: IndexedSeq[String]  = flatgraph.Accessors.getNodePropertyMulti[String](node.graph, nodeKind = node.nodeKind, propertyKind = 15, seq = node.seq)
}
final class AccessPropertyEvaluationStrategy(val node: nodes.StoredNode) extends AnyVal {
  def evaluationStrategy: String  = flatgraph.Accessors.getNodePropertySingle(node.graph, nodeKind = node.nodeKind, propertyKind = 16, seq = node.seq(), default = "<empty>": String)
}
final class AccessPropertyEvidenceDescription(val node: nodes.StoredNode) extends AnyVal {
  def evidenceDescription: IndexedSeq[String]  = flatgraph.Accessors.getNodePropertyMulti[String](node.graph, nodeKind = node.nodeKind, propertyKind = 17, seq = node.seq)
}
final class AccessPropertyExplicitAs(val node: nodes.StoredNode) extends AnyVal {
  def explicitAs: Option[Boolean]  = flatgraph.Accessors.getNodePropertyOption[Boolean](node.graph, nodeKind = node.nodeKind, propertyKind = 18, seq = node.seq)
}
final class AccessPropertyFilename(val node: nodes.StoredNode) extends AnyVal {
  def filename: String  = flatgraph.Accessors.getNodePropertySingle(node.graph, nodeKind = node.nodeKind, propertyKind = 19, seq = node.seq(), default = "<empty>": String)
}
final class AccessPropertyFullName(val node: nodes.StoredNode) extends AnyVal {
  def fullName: String  = flatgraph.Accessors.getNodePropertySingle(node.graph, nodeKind = node.nodeKind, propertyKind = 20, seq = node.seq(), default = "<empty>": String)
}
final class AccessPropertyGenericSignature(val node: nodes.StoredNode) extends AnyVal {
  def genericSignature: String  = flatgraph.Accessors.getNodePropertySingle(node.graph, nodeKind = node.nodeKind, propertyKind = 21, seq = node.seq(), default = "<empty>": String)
}
final class AccessPropertyHash(val node: nodes.StoredNode) extends AnyVal {
  def hash: Option[String]  = flatgraph.Accessors.getNodePropertyOption[String](node.graph, nodeKind = node.nodeKind, propertyKind = 22, seq = node.seq)
}
final class AccessPropertyImportedAs(val node: nodes.StoredNode) extends AnyVal {
  def importedAs: Option[String]  = flatgraph.Accessors.getNodePropertyOption[String](node.graph, nodeKind = node.nodeKind, propertyKind = 23, seq = node.seq)
}
final class AccessPropertyImportedEntity(val node: nodes.StoredNode) extends AnyVal {
  def importedEntity: Option[String]  = flatgraph.Accessors.getNodePropertyOption[String](node.graph, nodeKind = node.nodeKind, propertyKind = 24, seq = node.seq)
}
final class AccessPropertyIndex(val node: nodes.StoredNode) extends AnyVal {
  def index: Int  = flatgraph.Accessors.getNodePropertySingle(node.graph, nodeKind = node.nodeKind, propertyKind = 25, seq = node.seq(), default = -1: Int)
}
final class AccessPropertyInheritsFromTypeFullName(val node: nodes.StoredNode) extends AnyVal {
  def inheritsFromTypeFullName: IndexedSeq[String]  = flatgraph.Accessors.getNodePropertyMulti[String](node.graph, nodeKind = node.nodeKind, propertyKind = 26, seq = node.seq)
}
final class AccessPropertyIsExplicit(val node: nodes.StoredNode) extends AnyVal {
  def isExplicit: Option[Boolean]  = flatgraph.Accessors.getNodePropertyOption[Boolean](node.graph, nodeKind = node.nodeKind, propertyKind = 27, seq = node.seq)
}
final class AccessPropertyIsExternal(val node: nodes.StoredNode) extends AnyVal {
  def isExternal: Boolean  = flatgraph.Accessors.getNodePropertySingle(node.graph, nodeKind = node.nodeKind, propertyKind = 28, seq = node.seq(), default = false: Boolean)
}
final class AccessPropertyIsVariadic(val node: nodes.StoredNode) extends AnyVal {
  def isVariadic: Boolean  = flatgraph.Accessors.getNodePropertySingle(node.graph, nodeKind = node.nodeKind, propertyKind = 29, seq = node.seq(), default = false: Boolean)
}
final class AccessPropertyIsWildcard(val node: nodes.StoredNode) extends AnyVal {
  def isWildcard: Option[Boolean]  = flatgraph.Accessors.getNodePropertyOption[Boolean](node.graph, nodeKind = node.nodeKind, propertyKind = 30, seq = node.seq)
}
final class AccessPropertyKey(val node: nodes.StoredNode) extends AnyVal {
  def key: String  = flatgraph.Accessors.getNodePropertySingle(node.graph, nodeKind = node.nodeKind, propertyKind = 31, seq = node.seq(), default = "<empty>": String)
}
final class AccessPropertyLanguage(val node: nodes.StoredNode) extends AnyVal {
  def language: String  = flatgraph.Accessors.getNodePropertySingle(node.graph, nodeKind = node.nodeKind, propertyKind = 32, seq = node.seq(), default = "<empty>": String)
}
final class AccessPropertyLineNumber(val node: nodes.StoredNode) extends AnyVal {
  def lineNumber: Option[Int]  = flatgraph.Accessors.getNodePropertyOption[Int](node.graph, nodeKind = node.nodeKind, propertyKind = 33, seq = node.seq)
}
final class AccessPropertyLineNumberEnd(val node: nodes.StoredNode) extends AnyVal {
  def lineNumberEnd: Option[Int]  = flatgraph.Accessors.getNodePropertyOption[Int](node.graph, nodeKind = node.nodeKind, propertyKind = 34, seq = node.seq)
}
final class AccessPropertyMethodFullName(val node: nodes.StoredNode) extends AnyVal {
  def methodFullName: String  = flatgraph.Accessors.getNodePropertySingle(node.graph, nodeKind = node.nodeKind, propertyKind = 35, seq = node.seq(), default = "<empty>": String)
}
final class AccessPropertyModifierType(val node: nodes.StoredNode) extends AnyVal {
  def modifierType: String  = flatgraph.Accessors.getNodePropertySingle(node.graph, nodeKind = node.nodeKind, propertyKind = 36, seq = node.seq(), default = "<empty>": String)
}
final class AccessPropertyMyproperty(val node: nodes.StoredNode) extends AnyVal {
  def myproperty: String  = flatgraph.Accessors.getNodePropertySingle(node.graph, nodeKind = node.nodeKind, propertyKind = 37, seq = node.seq(), default = "": String)
}
final class AccessPropertyName(val node: nodes.StoredNode) extends AnyVal {
  def name: String  = flatgraph.Accessors.getNodePropertySingle(node.graph, nodeKind = node.nodeKind, propertyKind = 38, seq = node.seq(), default = "<empty>": String)
}
final class AccessPropertyOffset(val node: nodes.StoredNode) extends AnyVal {
  def offset: Option[Int]  = flatgraph.Accessors.getNodePropertyOption[Int](node.graph, nodeKind = node.nodeKind, propertyKind = 39, seq = node.seq)
}
final class AccessPropertyOffsetEnd(val node: nodes.StoredNode) extends AnyVal {
  def offsetEnd: Option[Int]  = flatgraph.Accessors.getNodePropertyOption[Int](node.graph, nodeKind = node.nodeKind, propertyKind = 40, seq = node.seq)
}
final class AccessPropertyOrder(val node: nodes.StoredNode) extends AnyVal {
  def order: Int  = flatgraph.Accessors.getNodePropertySingle(node.graph, nodeKind = node.nodeKind, propertyKind = 41, seq = node.seq(), default = -1: Int)
}
final class AccessPropertyOverlays(val node: nodes.StoredNode) extends AnyVal {
  def overlays: IndexedSeq[String]  = flatgraph.Accessors.getNodePropertyMulti[String](node.graph, nodeKind = node.nodeKind, propertyKind = 42, seq = node.seq)
}
final class AccessPropertyParserTypeName(val node: nodes.StoredNode) extends AnyVal {
  def parserTypeName: String  = flatgraph.Accessors.getNodePropertySingle(node.graph, nodeKind = node.nodeKind, propertyKind = 43, seq = node.seq(), default = "<empty>": String)
}
final class AccessPropertyPossibleTypes(val node: nodes.StoredNode) extends AnyVal {
  def possibleTypes: IndexedSeq[String]  = flatgraph.Accessors.getNodePropertyMulti[String](node.graph, nodeKind = node.nodeKind, propertyKind = 44, seq = node.seq)
}
final class AccessPropertyRoot(val node: nodes.StoredNode) extends AnyVal {
  def root: String  = flatgraph.Accessors.getNodePropertySingle(node.graph, nodeKind = node.nodeKind, propertyKind = 45, seq = node.seq(), default = "<empty>": String)
}
final class AccessPropertySignature(val node: nodes.StoredNode) extends AnyVal {
  def signature: String  = flatgraph.Accessors.getNodePropertySingle(node.graph, nodeKind = node.nodeKind, propertyKind = 46, seq = node.seq(), default = "": String)
}
final class AccessPropertyTypeDeclFullName(val node: nodes.StoredNode) extends AnyVal {
  def typeDeclFullName: String  = flatgraph.Accessors.getNodePropertySingle(node.graph, nodeKind = node.nodeKind, propertyKind = 47, seq = node.seq(), default = "<empty>": String)
}
final class AccessPropertyTypeFullName(val node: nodes.StoredNode) extends AnyVal {
  def typeFullName: String  = flatgraph.Accessors.getNodePropertySingle(node.graph, nodeKind = node.nodeKind, propertyKind = 48, seq = node.seq(), default = "<empty>": String)
}
final class AccessPropertyValue(val node: nodes.StoredNode) extends AnyVal {
  def value: String  = flatgraph.Accessors.getNodePropertySingle(node.graph, nodeKind = node.nodeKind, propertyKind = 49, seq = node.seq(), default = "": String)
}
final class AccessPropertyVersion(val node: nodes.StoredNode) extends AnyVal {
  def version: String  = flatgraph.Accessors.getNodePropertySingle(node.graph, nodeKind = node.nodeKind, propertyKind = 50, seq = node.seq(), default = "<empty>": String)
}
...
implicit def accessPropertyAliasTypeFullName(node: nodes.StoredNode & nodes.StaticType[nodes.HasAliasTypeFullNameEMT]): AccessPropertyAliasTypeFullName = new AccessPropertyAliasTypeFullName(node)
implicit def accessPropertyArgumentIndex(node: nodes.StoredNode & nodes.StaticType[nodes.HasArgumentIndexEMT]): AccessPropertyArgumentIndex = new AccessPropertyArgumentIndex(node)
implicit def accessPropertyArgumentName(node: nodes.StoredNode & nodes.StaticType[nodes.HasArgumentNameEMT]): AccessPropertyArgumentName = new AccessPropertyArgumentName(node)
implicit def accessPropertyAstParentFullName(node: nodes.StoredNode & nodes.StaticType[nodes.HasAstParentFullNameEMT]): AccessPropertyAstParentFullName = new AccessPropertyAstParentFullName(node)
implicit def accessPropertyAstParentType(node: nodes.StoredNode & nodes.StaticType[nodes.HasAstParentTypeEMT]): AccessPropertyAstParentType = new AccessPropertyAstParentType(node)
implicit def accessPropertyCanonicalName(node: nodes.StoredNode & nodes.StaticType[nodes.HasCanonicalNameEMT]): AccessPropertyCanonicalName = new AccessPropertyCanonicalName(node)
implicit def accessPropertyClosureBindingId(node: nodes.StoredNode & nodes.StaticType[nodes.HasClosureBindingIdEMT]): AccessPropertyClosureBindingId = new AccessPropertyClosureBindingId(node)
implicit def accessPropertyCode(node: nodes.StoredNode & nodes.StaticType[nodes.HasCodeEMT]): AccessPropertyCode = new AccessPropertyCode(node)
implicit def accessPropertyColumnNumber(node: nodes.StoredNode & nodes.StaticType[nodes.HasColumnNumberEMT]): AccessPropertyColumnNumber = new AccessPropertyColumnNumber(node)
implicit def accessPropertyColumnNumberEnd(node: nodes.StoredNode & nodes.StaticType[nodes.HasColumnNumberEndEMT]): AccessPropertyColumnNumberEnd = new AccessPropertyColumnNumberEnd(node)
implicit def accessPropertyContainedRef(node: nodes.StoredNode & nodes.StaticType[nodes.HasContainedRefEMT]): AccessPropertyContainedRef = new AccessPropertyContainedRef(node)
implicit def accessPropertyContent(node: nodes.StoredNode & nodes.StaticType[nodes.HasContentEMT]): AccessPropertyContent = new AccessPropertyContent(node)
implicit def accessPropertyControlStructureType(node: nodes.StoredNode & nodes.StaticType[nodes.HasControlStructureTypeEMT]): AccessPropertyControlStructureType = new AccessPropertyControlStructureType(node)
implicit def accessPropertyDependencyGroupId(node: nodes.StoredNode & nodes.StaticType[nodes.HasDependencyGroupIdEMT]): AccessPropertyDependencyGroupId = new AccessPropertyDependencyGroupId(node)
implicit def accessPropertyDispatchType(node: nodes.StoredNode & nodes.StaticType[nodes.HasDispatchTypeEMT]): AccessPropertyDispatchType = new AccessPropertyDispatchType(node)
implicit def accessPropertyDynamicTypeHintFullName(node: nodes.StoredNode & nodes.StaticType[nodes.HasDynamicTypeHintFullNameEMT]): AccessPropertyDynamicTypeHintFullName = new AccessPropertyDynamicTypeHintFullName(node)
implicit def accessPropertyEvaluationStrategy(node: nodes.StoredNode & nodes.StaticType[nodes.HasEvaluationStrategyEMT]): AccessPropertyEvaluationStrategy = new AccessPropertyEvaluationStrategy(node)
implicit def accessPropertyEvidenceDescription(node: nodes.StoredNode & nodes.StaticType[nodes.HasEvidenceDescriptionEMT]): AccessPropertyEvidenceDescription = new AccessPropertyEvidenceDescription(node)
implicit def accessPropertyExplicitAs(node: nodes.StoredNode & nodes.StaticType[nodes.HasExplicitAsEMT]): AccessPropertyExplicitAs = new AccessPropertyExplicitAs(node)
implicit def accessPropertyFilename(node: nodes.StoredNode & nodes.StaticType[nodes.HasFilenameEMT]): AccessPropertyFilename = new AccessPropertyFilename(node)
implicit def accessPropertyFullName(node: nodes.StoredNode & nodes.StaticType[nodes.HasFullNameEMT]): AccessPropertyFullName = new AccessPropertyFullName(node)
implicit def accessPropertyGenericSignature(node: nodes.StoredNode & nodes.StaticType[nodes.HasGenericSignatureEMT]): AccessPropertyGenericSignature = new AccessPropertyGenericSignature(node)
implicit def accessPropertyHash(node: nodes.StoredNode & nodes.StaticType[nodes.HasHashEMT]): AccessPropertyHash = new AccessPropertyHash(node)
implicit def accessPropertyImportedAs(node: nodes.StoredNode & nodes.StaticType[nodes.HasImportedAsEMT]): AccessPropertyImportedAs = new AccessPropertyImportedAs(node)
implicit def accessPropertyImportedEntity(node: nodes.StoredNode & nodes.StaticType[nodes.HasImportedEntityEMT]): AccessPropertyImportedEntity = new AccessPropertyImportedEntity(node)
implicit def accessPropertyIndex(node: nodes.StoredNode & nodes.StaticType[nodes.HasIndexEMT]): AccessPropertyIndex = new AccessPropertyIndex(node)
implicit def accessPropertyInheritsFromTypeFullName(node: nodes.StoredNode & nodes.StaticType[nodes.HasInheritsFromTypeFullNameEMT]): AccessPropertyInheritsFromTypeFullName = new AccessPropertyInheritsFromTypeFullName(node)
implicit def accessPropertyIsExplicit(node: nodes.StoredNode & nodes.StaticType[nodes.HasIsExplicitEMT]): AccessPropertyIsExplicit = new AccessPropertyIsExplicit(node)
implicit def accessPropertyIsExternal(node: nodes.StoredNode & nodes.StaticType[nodes.HasIsExternalEMT]): AccessPropertyIsExternal = new AccessPropertyIsExternal(node)
implicit def accessPropertyIsVariadic(node: nodes.StoredNode & nodes.StaticType[nodes.HasIsVariadicEMT]): AccessPropertyIsVariadic = new AccessPropertyIsVariadic(node)
implicit def accessPropertyIsWildcard(node: nodes.StoredNode & nodes.StaticType[nodes.HasIsWildcardEMT]): AccessPropertyIsWildcard = new AccessPropertyIsWildcard(node)
implicit def accessPropertyKey(node: nodes.StoredNode & nodes.StaticType[nodes.HasKeyEMT]): AccessPropertyKey = new AccessPropertyKey(node)
implicit def accessPropertyLanguage(node: nodes.StoredNode & nodes.StaticType[nodes.HasLanguageEMT]): AccessPropertyLanguage = new AccessPropertyLanguage(node)
implicit def accessPropertyLineNumber(node: nodes.StoredNode & nodes.StaticType[nodes.HasLineNumberEMT]): AccessPropertyLineNumber = new AccessPropertyLineNumber(node)
implicit def accessPropertyLineNumberEnd(node: nodes.StoredNode & nodes.StaticType[nodes.HasLineNumberEndEMT]): AccessPropertyLineNumberEnd = new AccessPropertyLineNumberEnd(node)
implicit def accessPropertyMethodFullName(node: nodes.StoredNode & nodes.StaticType[nodes.HasMethodFullNameEMT]): AccessPropertyMethodFullName = new AccessPropertyMethodFullName(node)
implicit def accessPropertyModifierType(node: nodes.StoredNode & nodes.StaticType[nodes.HasModifierTypeEMT]): AccessPropertyModifierType = new AccessPropertyModifierType(node)
implicit def accessPropertyMyproperty(node: nodes.StoredNode & nodes.StaticType[nodes.HasMypropertyEMT]): AccessPropertyMyproperty = new AccessPropertyMyproperty(node)
implicit def accessPropertyName(node: nodes.StoredNode & nodes.StaticType[nodes.HasNameEMT]): AccessPropertyName = new AccessPropertyName(node)
implicit def accessPropertyOffset(node: nodes.StoredNode & nodes.StaticType[nodes.HasOffsetEMT]): AccessPropertyOffset = new AccessPropertyOffset(node)
implicit def accessPropertyOffsetEnd(node: nodes.StoredNode & nodes.StaticType[nodes.HasOffsetEndEMT]): AccessPropertyOffsetEnd = new AccessPropertyOffsetEnd(node)
implicit def accessPropertyOrder(node: nodes.StoredNode & nodes.StaticType[nodes.HasOrderEMT]): AccessPropertyOrder = new AccessPropertyOrder(node)
implicit def accessPropertyOverlays(node: nodes.StoredNode & nodes.StaticType[nodes.HasOverlaysEMT]): AccessPropertyOverlays = new AccessPropertyOverlays(node)
implicit def accessPropertyParserTypeName(node: nodes.StoredNode & nodes.StaticType[nodes.HasParserTypeNameEMT]): AccessPropertyParserTypeName = new AccessPropertyParserTypeName(node)
implicit def accessPropertyPossibleTypes(node: nodes.StoredNode & nodes.StaticType[nodes.HasPossibleTypesEMT]): AccessPropertyPossibleTypes = new AccessPropertyPossibleTypes(node)
implicit def accessPropertyRoot(node: nodes.StoredNode & nodes.StaticType[nodes.HasRootEMT]): AccessPropertyRoot = new AccessPropertyRoot(node)
implicit def accessPropertySignature(node: nodes.StoredNode & nodes.StaticType[nodes.HasSignatureEMT]): AccessPropertySignature = new AccessPropertySignature(node)
implicit def accessPropertyTypeDeclFullName(node: nodes.StoredNode & nodes.StaticType[nodes.HasTypeDeclFullNameEMT]): AccessPropertyTypeDeclFullName = new AccessPropertyTypeDeclFullName(node)
implicit def accessPropertyTypeFullName(node: nodes.StoredNode & nodes.StaticType[nodes.HasTypeFullNameEMT]): AccessPropertyTypeFullName = new AccessPropertyTypeFullName(node)
implicit def accessPropertyValue(node: nodes.StoredNode & nodes.StaticType[nodes.HasValueEMT]): AccessPropertyValue = new AccessPropertyValue(node)
implicit def accessPropertyVersion(node: nodes.StoredNode & nodes.StaticType[nodes.HasVersionEMT]): AccessPropertyVersion = new AccessPropertyVersion(node)
}
...
implicit def accessAnnotationliteralbase(node: nodes.AnnotationLiteralBase): AccessAnnotationliteralBase = new AccessAnnotationliteralBase(node)
implicit def accessAnnotationparameterbase(node: nodes.AnnotationParameterBase): AccessAnnotationparameterBase = new AccessAnnotationparameterBase(node)
implicit def accessAnnotationparameterassignbase(node: nodes.AnnotationParameterAssignBase): AccessAnnotationparameterassignBase = new AccessAnnotationparameterassignBase(node)
implicit def accessArrayinitializerbase(node: nodes.ArrayInitializerBase): AccessArrayinitializerBase = new AccessArrayinitializerBase(node)
...
implicit def accessClosurebindingbase(node: nodes.ClosureBindingBase): AccessClosurebindingBase = new AccessClosurebindingBase(node)
...
implicit def accessControlstructurebase(node: nodes.ControlStructureBase): AccessControlstructureBase = new AccessControlstructureBase(node)
implicit def accessDependencybase(node: nodes.DependencyBase): AccessDependencyBase = new AccessDependencyBase(node)
implicit def accessFieldidentifierbase(node: nodes.FieldIdentifierBase): AccessFieldidentifierBase = new AccessFieldidentifierBase(node)
...
implicit def accessKeyvaluepairbase(node: nodes.KeyValuePairBase): AccessKeyvaluepairBase = new AccessKeyvaluepairBase(node)
...
implicit def accessMethodparameterinbase(node: nodes.MethodParameterInBase): AccessMethodparameterinBase = new AccessMethodparameterinBase(node)
implicit def accessMethodparameteroutbase(node: nodes.MethodParameterOutBase): AccessMethodparameteroutBase = new AccessMethodparameteroutBase(node)
implicit def accessMethodrefbase(node: nodes.MethodRefBase): AccessMethodrefBase = new AccessMethodrefBase(node)
implicit def accessMethodreturnbase(node: nodes.MethodReturnBase): AccessMethodreturnBase = new AccessMethodreturnBase(node)
...
implicit def accessNamespaceblockbase(node: nodes.NamespaceBlockBase): AccessNamespaceblockBase = new AccessNamespaceblockBase(node)
...
implicit def accessTagnodepairbase(node: nodes.TagNodePairBase): AccessTagnodepairBase = new AccessTagnodepairBase(node)
implicit def accessTemplatedombase(node: nodes.TemplateDomBase): AccessTemplatedomBase = new AccessTemplatedomBase(node)
implicit def accessTypebase(node: nodes.TypeBase): AccessTypeBase = new AccessTypeBase(node)
implicit def accessTypeargumentbase(node: nodes.TypeArgumentBase): AccessTypeargumentBase = new AccessTypeargumentBase(node)
implicit def accessTypedeclbase(node: nodes.TypeDeclBase): AccessTypedeclBase = new AccessTypedeclBase(node)
implicit def accessTypeparameterbase(node: nodes.TypeParameterBase): AccessTypeparameterBase = new AccessTypeparameterBase(node)
