...
*▸ ArgumentIndex (Int); Cardinality `one` (mandatory with default value `-1`); AST-children of CALL nodes have an argument index, that is used to match
...
*▸ ArgumentName (String); Cardinality `ZeroOrOne` (optional); For calls involving named parameters, the `ARGUMENT_NAME` field holds the
...
*▸ DispatchType (String); Cardinality `one` (mandatory with default value `<empty>`); This field holds the dispatch type of a call, which is either `STATIC_DISPATCH` or
...
*▸ MethodFullName (String); Cardinality `one` (mandatory with default value `<empty>`); The FULL_NAME of a method. Used to link CALL and METHOD nodes. It is required
...
class Call(graph_4762: flatgraph.Graph, seq_4762: Int) extends StoredNode(graph_4762, 7, seq_4762) with CallBase with CallRepr with Expression with StaticType[CallEMT] {
