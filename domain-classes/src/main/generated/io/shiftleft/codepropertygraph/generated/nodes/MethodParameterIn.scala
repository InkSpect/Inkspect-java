...
*▸ ClosureBindingId (String); Cardinality `ZeroOrOne` (optional); Identifier which uniquely describes a CLOSURE_BINDING. This property is used to match captured LOCAL nodes with the corresponding CLOSURE_BINDING nodes
...
*▸ EvaluationStrategy (String); Cardinality `one` (mandatory with default value `<empty>`); For formal method input parameters, output parameters, and return parameters,
...
*▸ Index (Int); Cardinality `one` (mandatory with default value `-1`); Specifies an index, e.g., for a parameter or argument.
 Explicit parameters are numbered from 1 to N, while index 0 is reserved for implicit
 self / this parameter.

*▸ IsVariadic (Boolean); Cardinality `one` (mandatory with default value `false`); Specifies whether a parameter is the variadic argument handling parameter of
...
class MethodParameterIn(graph_4762: flatgraph.Graph, seq_4762: Int) extends StoredNode(graph_4762, 26, seq_4762) with MethodParameterInBase with CfgNode with Declaration with StaticType[MethodParameterInEMT] {
